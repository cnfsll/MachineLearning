...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr 20 20:59:40 2017
...
np.save('train_acc_dropout.npy', train_acc)
np.save('test_acc_dropout.npy', test_acc_mean)
#画图,比较使用和不使用dropout时收敛情况
train_acc_nondrop = np.load('train_acc_adam.npy')
plt.plot(epoch, train_acc, 'r-', label='dropout')
plt.plot(epoch, train_acc_nondrop, 'b-', label='non-dropout')
plt.title('The change of accuracy as the epoch increases with and without dropout')
plt.ylabel('Accuracy')
plt.xlabel('epcoh')
plt.legend()
plt.show()
